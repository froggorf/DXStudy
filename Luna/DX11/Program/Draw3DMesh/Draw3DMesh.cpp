...
class Draw3DMeshApp : public D3DApp
{
public:
	Draw3DMeshApp(HINSTANCE hInstance);
	~Draw3DMeshApp();
				   ...
				   PSTR cmdLine, int showCmd)
{
	...
	Draw3DMeshApp theApp(hInstance);
...
Draw3DMeshApp::Draw3DMeshApp(HINSTANCE hInstance)
: D3DApp(hInstance) 
{
...
Draw3DMeshApp::~Draw3DMeshApp()
{
	// ComPtr
}

bool Draw3DMeshApp::Init()
{
	if(!D3DApp::Init())
		return false;
...
void Draw3DMeshApp::UpdateScene(float dt)
{
	...
	XMVECTOR Pos = XMVectorSet(0.0f,3.0f,-5.0f,1.0f);
...
void Draw3DMeshApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	m_LastMousePos.x = x;
	m_LastMousePos.y = y;
...
void Draw3DMeshApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Draw3DMeshApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void Draw3DMeshApp::BuildGeometryBuffers()
{
	// 버텍스 버퍼
	Vertex vertices[] =
    {
...
void Draw3DMeshApp::BuildShader()
{
