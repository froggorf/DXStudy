...
public:
	...
	static void LoadModelData(const std::string& path, Microsoft::WRL::ComPtr<ID3D11Device> pDevice, std::vector<std::vector<MyVertexData>>& AllVertices,std::vector<std::vector<UINT>>& AllIndices, std::vector<Microsoft::WRL::ComPtr<ID3D11Buffer>>& pVertexBuffer, std::vector<Microsoft::WRL::ComPtr<ID3D11Buffer>>& pIndexBuffer, float& BoundSphereLength);
	// 한 메쉬가 하나의 오브젝트로 존재시
	...
	static void LoadSkeletalModelData(const std::string& path, Microsoft::WRL::ComPtr<ID3D11Device> pDevice, std::vector<Microsoft::WRL::ComPtr<ID3D11Buffer>>& pVertexBuffer, std::vector<Microsoft::WRL::ComPtr<ID3D11Buffer>>& pIndexBuffer, std::map<std::string, BoneInfo>& modelBoneInfoMap, float& BoundSphereLength);
...
private:
	...
	static void ExtractBoneWeightForVertices(std::vector<MyVertexData>& vVertexData, aiMesh* mesh, std::map<std::string, BoneInfo>& ModelBoneInfoMap,aiNode* RootNode);
