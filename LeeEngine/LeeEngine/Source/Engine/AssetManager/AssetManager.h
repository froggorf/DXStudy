...
public:
	...
	static std::shared_ptr<UTexture> CreateTexture(const std::string& Name, UINT        Width, UINT Height,
													DXGI_FORMAT       PixelFormat, UINT BindFlag,
...
private:
	...
	static void ExtractBoneWeightForVertices(std::vector<MyVertexData>&      vVertexData, aiMesh* mesh,
											std::map<std::string, BoneInfo>& modelBoneInfoMap);
