...
class UBlendSpace : public UAnimationAsset
{
	...
	virtual void GetAnimationBoneTransforms(const XMFLOAT2& AnimValue, float CurrentAnimTime, std::vector<FBoneLocalTransform>& OutBoneTransforms, std::vector<FAnimNotifyEvent>& OutActiveNotifies);
...
private:
	...
	static void TriangleInterpolation(const XMFLOAT2& AnimValue, const std::shared_ptr<FAnimClipTriangle>& Triangle, float AnimTime, std::vector<FBoneLocalTransform>& OutBoneTransforms, std::vector<FAnimNotifyEvent>& OutActiveNotifies);
	static void LinearInterpolation(const XMFLOAT2& CurrentValue, const std::shared_ptr<FAnimClipEdge>& Edge, float AnimTime, std::vector<FBoneLocalTransform>& OutBoneTransforms, std::vector<FAnimNotifyEvent>& OutActiveNotifies);
	static void CalculateOneAnimation(const std::shared_ptr<FAnimClipPoint>& Point, float AnimTime, std::vector<FBoneLocalTransform>& OutBoneTransforms, std::vector<FAnimNotifyEvent>& OutActiveNotifies);
