...
//}
//
//void UAimOffsetBlendSpace::GetAnimationBoneTransforms(const XMFLOAT2& AnimValue, float CurrentAnimTime, std::vector<XMMATRIX>& OutMatrices, std::vector<FAnimNotifyEvent>& OutActiveNotifies)
//{
//	std::vector<XMMATRIX> AO_Matrices(MAX_BONES, XMMatrixIdentity());
//	UBlendSpace::GetAnimationBoneTransforms(AnimValue, CurrentAnimTime, OutMatrices, OutActiveNotifies);
//
...
//	UBlendSpace::GetAnimationBoneTransforms(XMFLOAT2{(HorizontalValue.x + HorizontalValue.y) / 2, (VerticalValue.x + VerticalValue.y) / 2}, 0.0f, DefaultAnimMatrices, Dummy);
//}
