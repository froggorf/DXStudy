...
{
	UINT PrimitiveID;
	std::function<void(const std::shared_ptr<class FScene>&)> CommandLambda;
	FRenderTask* Next;
...
public:
	static void Enqueue(UINT PrimitiveId, const std::function<void(const std::shared_ptr<class FScene>&)>& CommandLambda)
	{
		// 다중 생성 기반 Queue
...
public:
	static void Execute()
	{
		MY_LOG("TEST",EDebugLogLevel::DLL_Display,"EXECUTE");
